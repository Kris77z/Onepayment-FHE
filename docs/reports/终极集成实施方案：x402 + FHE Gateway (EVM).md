...
| | | **实现细节**：<br>- FHE + x402 组合支付流程：先通过 x402 进行实际的 USDC 转账（Gasless），再将加密金额存储到 FHE 合约<br>- 前端传递 FHE 加密金额作为 x402 请求元数据（`X-FHE-ENCRYPTED` 头）<br>- 交易记录包含 x402 交易哈希（主要）和 FHE 存储交易哈希 | |
...
| | | **实现细节**：<br>- FHE + x402 组合支付：先 x402 转账（Gasless），再 FHE 存储（隐私）<br>- 前端通过 `X-FHE-ENCRYPTED` 请求头传递加密金额<br>- 双重交易记录：x402 交易哈希（主要）+ FHE 存储交易哈希 | |
